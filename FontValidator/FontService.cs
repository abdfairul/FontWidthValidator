...
namespace FontValidator
{
    public class FontService : IDisposable
    {
        ...
        internal void SetSize(float size)
        {
            _size = size;
            if (FontFace != null)
                FontFace.SetCharSize(0, size, 0, 72);
        }
        ...
        internal static Bitmap RenderString(Library library, Face face,
            string text, Color foreColor, Color backColor)
        {
        ...
        internal virtual KeyValuePair<float, float> MeasureString(string text, float fontsize)
        {
            Library library = this.lib;
            Face face = this.FontFace;
            ...
            return new KeyValuePair<float, float>(stringWidth, stringHeight);
        }
        #endregion
